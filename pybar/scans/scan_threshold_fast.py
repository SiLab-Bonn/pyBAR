    ...
    def scan(self):
        ...
        while self.scan_parameter_value <= scan_parameter_range[1]:  # scan as long as scan parameter is smaller than defined maximum
            ...
            if not self.start_condition_triggered or self.data_points > self.curr_minimum_data_points:  # speed up, only create histograms when needed. Python is much too slow here.
                ...
                col, row = convert_data_array(array=self.read_data(), filter_func=logical_and(is_fe_word, is_data_record), converter_func=get_col_row_array_from_data_record_array)
