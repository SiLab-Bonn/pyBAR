...
class FastThresholdScan(Fei4RunBase):
    ...
    def scan(self):
        ...
        while self.scan_parameter_value <= scan_parameter_range[1]:  # scan as long as scan parameter is smaller than defined maximum
            ...
            with self.readout(PlsrDAC=self.scan_parameter_value, reset_fifo=True, fill_buffer=True, callback=self.handle_data if self.record_data else None):
            ...
            if not self.start_condition_triggered or self.data_points > self.minimum_data_points:  # speed up, only create histograms when needed. Python is much too slow here.
                ...
                col, row = self.fifo_readout.get_raw_data_from_buffer(filter_func=logical_and(is_fe_word, is_data_record), converter_func=get_col_row_array_from_data_record_array)[0]
...
if __name__ == "__main__":
    with RunManager('configuration.yaml') as runmngr:
        runmngr.run_run(FastThresholdScan)
