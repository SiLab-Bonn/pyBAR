...
class NoiseOccupancyScan(Fei4RunBaseParallel):
    ...
    _default_run_conf = {
        ...
        "n_triggers": 100000,  # total number of triggers which will be sent to the FE. From 1 to 4294967295 (32-bit unsigned int).
...
#         self.register.set_global_register_value("Trig_Lat", self.trigger_latency)  # set trigger latency
    ...
    def scan(self):
        # preload command
        lvl1_command = self.register.get_commands("LV1")[0] + self.register.get_commands("zeros", length=60)[0] + self.register.get_commands("LV1")[0] + self.register.get_commands("zeros", length=self.trigger_rate_limit)[0] + self.register.get_commands("zeros", length=5000)[0]
