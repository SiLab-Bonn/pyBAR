...
class ExtTriggerScanSHiP(Fei4RunBase):
    ...
    _default_run_conf = {
        ...
        "threaded_scan": True,
        "trig_count": 8,  # FE-I4 trigger count, number of consecutive BCs, 0 means 16, from 0 to 15
        ...
        with self.readout(no_data_timeout=self.no_data_timeout, **self.scan_parameters._asdict()):
            with self.trigger():
                ...
                while not self.stop_run.wait(1.0):
                    self.fifo_readout.print_fifo_status()
                    if not got_data:
...
if __name__ == "__main__":
    with RunManager('../configuration.yaml') as runmngr:
        runmngr.run_run(ExtTriggerScanSHiP)
