...
class DigitalScanParallel(Fei4RunBaseParallel):
    '''Digital scan
    '''
    _default_run_conf = {
    ...
    def scan(self):
        with self.readout():
#            self._conf['multiple_fes_configuration'][self.fe_number] = FEI4Register(fe_type=self.conf['fe_flavor'], chip_address=0, broadcast=True)
...
if __name__ == "__main__":
    RunManager('../configuration.yaml').run_run(DigitalScanParallel)
