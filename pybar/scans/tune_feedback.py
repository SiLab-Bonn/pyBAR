...
from pybar.daq.readout_utils import convert_data_array, is_fe_word, is_data_record, logical_and, get_col_row_tot_array_from_data_record_array
...
class FeedbackTuning(Fei4RunBase):
    ...
    def scan(self):
        ...
        for feedback_bit in feedback_tune_bits:
            ...
            data = convert_data_array(array=self.read_data(), filter_func=logical_and(is_fe_word, is_data_record), converter_func=get_col_row_tot_array_from_data_record_array)
        ...
        if not self.stop_run.is_set():
            ...
            if abs(mean_tot - self.target_tot) > abs(tot_mean_best - self.target_tot):
                if self.register.get_global_register_value("PrmpVbpf") != feedback_best:
    ...
    def set_prmp_vbpf_bit(self, bit_position, bit_value):
        commands = []
        commands.extend(self.register.get_commands("ConfMode"))
        if bit_value == 1:
