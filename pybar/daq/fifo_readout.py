...
class FifoReadout(object):
    ...
    def get_rx_enable_status(self, channels=None):
        if channels:
            ...
            return list(map(lambda channel: True if (channel.ENABLE_RX or (self.enabled_fe_channels is not None and channel.name in self.enabled_fe_channels)) else False, self.dut.get_modules('fei4_rx')))
