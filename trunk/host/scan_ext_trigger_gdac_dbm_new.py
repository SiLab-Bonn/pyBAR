...
#     gdacs = list(set(gdacs).difference(range(66, 245)))
#     gdacs.sort()
...
scan_configuration = {
    "gdacs": gdacs,
    "mode": 2,
    "trigger_latency": 254 - 1 * 16 - 53,
    "trigger_delay": 50,
    "col_span": [2, 77],
    "row_span": [2, 335],
    "timeout_no_data": 5 * 60 * 60,
    "scan_timeout": 20 * 60,
    "max_triggers": 10000000,
    "enable_hitbus": False
}
...
class ExtTriggerGdacScan(ScanBase):
    def __init__(self, configuration_file, definition_file=None, bit_file=None, force_download=False, device=None, scan_data_path=None, device_identifier=""):
        super(ExtTriggerGdacScan, self).__init__(configuration_file=configuration_file, definition_file=definition_file, bit_file=bit_file, force_download=force_download, device=device, scan_data_path=scan_data_path, device_identifier=device_identifier, scan_identifier="ext_trigger_gdac_scan_dbm")
