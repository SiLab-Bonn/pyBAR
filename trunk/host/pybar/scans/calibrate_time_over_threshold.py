...
class TimeOverThresholdScan(ScanBase):
    ...
    def scan(self):
        ...
        if not self.scan_parameter_range:
            scan_parameter_range = range(0, (2 ** self.register.global_registers[self.scan_parameter]['bitlength']) - 1)
        ...
        with open_raw_data_file(filename=self.scan_data_filename, title=self.scan_id, scan_parameters=[self.scan_parameter]) as raw_data_file:
            for index, scan_parameter_value in enumerate(scan_parameter_range):
                ...
                col_row_mean_tot_hist = np.ma.array(np.average(col_row_tot_hist, axis=2, weights=range(0, 16)) * sum(range(0, 16)) / self.repeat_command, mask=np.all(col_row_tot_hist == 0, axis=2))
                plotThreeWay(hist=col_row_mean_tot_hist.T, title='Mean ToT (%s = %d)' % (self.scan_parameter, scan_parameter_value), x_axis_title='ToT', minimum=0, maximum=15, filename=output_pdf)
                # saving data
            ...
            plot_scurves(occupancy_hist=tot_calibration, title='ToT to Charge Calibration', scan_parameters=scan_parameter_range, scan_parameter_name=self.scan_parameter, ylabel='ToT', max_occ=15, filename=output_pdf)
            output_pdf.close()
