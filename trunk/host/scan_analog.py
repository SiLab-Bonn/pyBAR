...
if __name__ == "__main__":
    ...
    commands = []
    scan.register.set_global_register_value("PlsrDAC", 200)
    commands.extend(scan.register.get_commands("wrregister", name = ["PlsrDAC"]))
    scan.register_utils.send_commands(commands)
    
    import cProfile
    pr = cProfile.Profile()
    repeat = 100
    cal_lvl1_command = scan.register.get_commands("cal")[0]+BitVector.BitVector(size = 40)+scan.register.get_commands("lv1")[0]+BitVector.BitVector(size = 1000)
    pr.enable()
    scan.scan_utils.base_scan(cal_lvl1_command, repeat = repeat, mask = 6, dcs = [], same_mask_for_all_dc = True, hardware_repeat = True, digital_injection = False, read_function = None)#scan.readout.read_once)
    pr.disable()
