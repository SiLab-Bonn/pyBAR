...
#     'input_file_calibration': 'data//MDBM30//calibrate_threshold_gdac_MDBM30.h5',
    "analysis_steps": [1,2],  #, 2, 3, 4],  # the analysis includes the selected steps here. See explanation above.
    "chip_flavor": 'fei4a',
    "n_bcid": 4,
    ...
    "col_span": [5, 65],  # the column pixel range to use in the analysis
    "row_span": [10, 300],  # the row pixel range to use in the analysis
    "min_cut_threshold": 500,  # the minimum cut threshold for the occupancy to define pixel to use in the analysis
    "max_cut_threshold": None,  # the minimum cut threshold for the occupancy to define pixel to use in the analysis
    ...
    "min_thr": 2000,  # minimum threshold position in gdac setting to be used for the analysis
    "max_thr": 35000,  # maximum threshold position in gdac setting to be used for the analysis
    "plot_normalization": False,  # active the output of the normalization
    "plot_cluster_sizes": False,
...
def analyze_raw_data_v2(input_files, output_file_hits, chip_flavor, scan_parameter):
...
def analyse_selected_hits_v2(input_file_hits, output_file_hits, output_file_hits_analyzed, scan_data_filenames, cluster_size_condition='cluster_size==1', n_cluster_condition='n_cluster==1'):
#     logging.info('Analyze selected hits with ' + cluster_size_condition + ' and ' + n_cluster_condition + ' for ' + str(len(input_file_hits)) + ' hit file(s)')
#     if os.path.isfile(output_file_hits) and not analysis_configuration["overwrite_output_files"]:  # skip analysis if already done
...
#     if os.path.isfile(output_file_hits_analyzed):  # skip analysis if already done
...
def analyse_selected_hits(input_files_hits, output_files_hits, output_files_hits_analyzed, scan_data_filenames, cluster_size_condition='cluster_size==1', n_cluster_condition='n_cluster==1'):
    ...
    for index in range(0, len(input_files_hits)):  # loop over all hit files
        if os.path.isfile(output_files_hits[index]) and not analysis_configuration["overwrite_output_files"]:  # skip analysis if already done
            ...
            analysis.select_hits_from_cluster_info(input_file_hits=input_files_hits[index], output_file_hits=output_files_hits[index], cluster_size_condition=cluster_size_condition, n_cluster_condition=n_cluster_condition, output_pdf=None)  # select hits and copy the mto new file
...
if __name__ == "__main__":
    data_files = analysis_utils.get_data_file_names_from_scan_base(analysis_configuration['scan_name'], filter_file_words=['interpreted', 'cut_', 'cluster_sizes', 'trigger_fe'], parameter=True)
    files_dict = analysis_utils.get_parameter_from_files(data_files, parameters='GDAC')  # get a sorted ordered dict with GDAC, raw_data_filename
    logging.info('Found ' + str(len(files_dict)) + ' raw data files.')

    hit_file = analysis_configuration['scan_name'][0] + '_interpreted.h5'
    hit_cut_files = [filename[:-3] + '_cut_hits.h5' for filename in files_dict.keys()]
    hit_cut_file = analysis_configuration['scan_name'][0] + '_cut_hits.h5'
    hit_analyzed_file = analysis_configuration['scan_name'][0] + '_analyzed.h5'
    hit_analyzed_files = [filename[:-3] + '_analyzed.h5' for filename in files_dict.keys()]
...
#
    if 1 in analysis_configuration['analysis_steps']:
        analyze_raw_data_v2(input_files=files_dict.keys(), output_file_hits=hit_file, chip_flavor=analysis_configuration['chip_flavor'], scan_parameter='GDAC')
    if 2 in analysis_configuration['analysis_steps'] and not 2.5 in analysis_configuration['analysis_steps']:
#         analyse_selected_hits(input_files_hits=hit_file, output_files_hits=hit_cut_files, output_files_hits_analyzed=hit_analyzed_files, scan_data_filenames=scan_data_filenames)
        analyse_selected_hits_v2(input_file_hits=hit_file, output_file_hits=hit_cut_file, output_file_hits_analyzed=hit_analyzed_files, scan_data_filenames=analysis_configuration['scan_name'][0])
#     if 2.5 in analysis_configuration['analysis_steps']:
#         analysis.histogram_cluster_table(hit_files)
#     if 3 in analysis_configuration['analysis_steps'] and not 2.5 in analysis_configuration['analysis_steps']:
#         analysis.analyze_cluster_size_per_scan_parameter(input_files_hits=hit_files, output_file_cluster_size=cluster_sizes_file, parameter='GDAC', overwrite_output_files=analysis_configuration['overwrite_output_files'], output_pdf=False)
#     print(hit_analyzed_files)
...
#             two = in_file_h5.root.HistOcc[:]
#             print np.all(one == two)
#             print np.sum(one[20, 20])
#             print np.sum(two[20, 20])
#             print np.all(np.in1d(one[20, 20], two[20, 20]))
#             print one.dtype
#             print two.dtype
#         with tb.open_file('data//SCC_99//20V//tt.h5', "w") as out_file_h5:
#             occupancy_array_table = out_file_h5.createCArray(out_file_h5.root, name='HistOcc', title='Occupancy Histogram', atom=tb.Atom.from_dtype(one.dtype), shape=(336, 80, one.shape[2]), filters=tb.Filters(complib='blosc', complevel=5, fletcher32=False))
#             occupancy_array_table[0:336, 0:80, 0:one.shape[2]] = one
