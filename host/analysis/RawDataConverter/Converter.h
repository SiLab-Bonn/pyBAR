...
public:
  ...
  //main function
  bool convertTable(const std::string& FileName);                          //starts the converting: reads the file in chunks and stores the converted data for each read chunk into a new file
  ...
  void setOccHistName(const std::string& OccupancyHistName);               //name of the occupancy histogram node
  void setRelBcidHistName(const std::string& RelBcidHistName);             //name of the relative BCID histogram node
  void setTotHistName(const std::string& TotHistName);                     //name of the TOT histogram node
  ...
  void setErrorOutput(bool Toggle = true);								//set to see errors output, standard setting: on
	void setWarningOutput(bool Toggle = true);							//set to see warnings output, standard setting: on
...
private:
  ...
  bool loadHDF5file(const std::string& FileName);         //path+file name of the HDF5 file
  ...
  void writeTotHist();              //writes the Tot array into the output file
  void writeRelBcidHist();          //writes the relative bcid array into the output file
  ...
  std::string _tableNameHits, _tableNameMeta, _occHistName, _parHistName, _threshHistName, _noiseHistName, _triggerErrorHistName, _errorHistName, _sRhistName, _relBcidHistName, _totHistName; //node names for different output data

  bool _createOutFile, _createHitsTable, _createMetaData, _createParData, _createOccHist, _createThresholdHists, _createTriggerErrorHist, _createErrorHist, _createSRhist, _createRelBcidHist, _createTotHist; //flags for different analysis options
