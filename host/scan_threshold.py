    ...
    def scan(self, mask = 3, repeat = 100, scan_parameter = 'PlsrDAC', scan_paramter_value = None):
        ...
        with open_raw_data_file(filename = self.scan_data_filename, title=self.scan_identifier, scan_parameters=[scan_parameter]) as raw_data_file:
            ...
            for scan_paramter_value in scan_paramter_value_list:
                ...
                self.scan_loop(cal_lvl1_command, repeat = repeat, mask = mask, mask_steps = [], double_columns = [], same_mask_for_all_dc = True, hardware_repeat = True, digital_injection = False, eol_function = None)#self.readout.read_once)
