...
def select_hits():
    ...
    with tb.openFile(input_file_hits, mode="r") as in_hit_file_h5:
        with tb.openFile(output_file_hits, mode="w") as out_hit_file_h5:
            ...
            selected_events_1 = get_events_with_cluster_size(cluster_table, condition=cluster_size_condition)  # select the events with clusters of a certain size
            selected_events_2 = get_events_with_n_cluster(cluster_table, condition=n_cluster_condition)  # select the events with a certain cluster number
